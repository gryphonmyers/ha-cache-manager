...
module.exports = class FileStore extends CacheStore {
    ...
    async mset() {
        ...
        return Promise.all([
            ...pairs.map((pair, ii) => fs.writeFile(path.join(this.path, `${pair[0]}_meta.json`), JSON.stringify({ setTime: timestamps[ii] }))),
    ...
    async mdel() {
        ...
        return Promise.all([
            ...
            ...keys.map((key, ii) => fs.unlink(path.join(this.path, `${key}.json`)))
        ])
    }

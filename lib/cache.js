...
module.exports = class Cache extends EventEmitter {
    ...
    load(values) {
        ...
        return (this.store ? this.store.values()
            .then(data => {
                this.lru.load(data.filter(val => val));
    ...
    async get(key) {
        if (this.store) {
            var remoteState = await (this.diffPromises[key] || (this.diffPromises[key] = this.diffStoreState(this.store, key))).then(remoteState => {
            ...
            switch (remoteState) {
                case REMOTE_DELETED:
                    var refreshPromise = (this.refreshPromises[key] || (this.refreshPromises[key] = (async () => {
                        await this.del(key);
                        this.refreshPromises[key] = null;
                        return null;
                    })()))
                    ...
                    return null;
                case REMOTE_CHANGED:
                    refreshPromise = (this.refreshPromises[key] || (this.refreshPromises[key] = (async () => {
                        var val = await this.store.get(key);
                        await this.set(val);
                        this.refreshPromises[key] = null;
                        return val;
                    })()))

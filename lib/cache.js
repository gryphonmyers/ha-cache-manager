...
module.exports = class Cache extends EventEmitter {
    ...
    async wrap(key, fetcher, opts={}) {
        ...
        if (this.refreshPromises[key]) {
            var promise = this.refreshPromises[key]
                .then(async () => {
                    var val = await this.get(key)
                    return val != null ? val : lastGoodValue;
                });
        } else {
            promise = this.wrapPromises[key] || (this.wrapPromises[key] = Promise.resolve(fetcher(lastGoodValue))
                .then(async val => {
